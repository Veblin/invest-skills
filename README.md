...
Windows 用户若改用 `git clone`：默认 `core.symlinks=false` 会把仓库 32 条技能链接物化成文本文件，导致技能发现失效。运行重建脚本（NTFS junction 重建 24 个目录链接 + 硬链接重建 8 个 commands 文件，无需管理员权限，幂等；数量与 `skills/` 目录一一对应）：

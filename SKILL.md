...
name: invest-A
version: "0.1.0"
description: "辅助用户做投资学习的 AI 技能。覆盖 A 股/港股个股调研的七大维度，产出带数据引用的 Markdown 学习报告。这是一个学习工具，不是决策工具——不提供买卖建议、目标价或仓位建议。"
...
**LAW 7** — 关键财务数据必须标注原始来源和时间戳。引用格式：`[来源: {接口名} / {获取日期}]`。
...
✅ "当前 PE(TTM) 为 25.3x，处于近 5 年 30.1% 分位。[来源: akshare.stock_individual_info_em / 2026-06-09]"
✅ "ROE 从 2020 年的 18.2% 下降至 2024 年的 12.1%。[来源: akshare.stock_financial_abstract / 2026-06-09；同花顺 iFinD 交叉核对一致]"
✅ "以下三个假设如有变化，结论将发生显著改变...(1)永续增长率 3% [假设依据: 近5年行业增速均值，来源: 中国酒业协会2025年报]"
✅ "北向资金近20日净流出42.3亿元。[来源: akshare.stock_hsgt_hist_em / 2026-06-09] ⚠️ 北向资金数据有 T+1 延迟"
...
对标学术论文标准，本 Skill 使用统一的引用格式：

```
事实性陈述（财务数据、行情数据、机构持仓等）：
  "数值" [来源: {数据接口名} / {获取日期}]
  "数值" [来源: {数据接口名} / {获取日期}；{交叉核验来源}]
  "数值" [来源: {财报文件名} 第{页码}页 / {发布日期}]
...
> **Phase 0 骨架已就绪。** 数据采集管道（Phase 1）尚未实现。请先执行 `pip install -r requirements.txt` 安装依赖，配置 `.env.example` 中的环境变量后，Skill 即可端到端工作。

...
落盘: reports/market-pulse/{YYYY-MM-DD}.md（mkdir -p 后写入全文）
       ↓
复检: 第 0 层机器准出（report_qc --fail-on error，非 FAIL）
      → 三层人工复检（数字/合规/逻辑）通过后
       ↓
...
> **LAW 17 强制（下述模板已内嵌骨架）**：每个 `[分析]` 块**首句**必须是 `**结论：**` 加粗主旨句，
> 块内须含 `**逻辑链：**`。两条都必须是**字面量本身**（冒号后紧接 `**`）——
> 写成 `**结论：内容**` 不会被 lint 识别，这是 2026-09-18 报告的实测返工点。
> 依据：`skills/lib/references/report-conventions.md` §1.1（简报铁律）。
...
**[分析]**
**结论：** {一句话核心判断 — 如「估值处历史中位，不构成极端信号也不提供厚缓冲」}
...
**[分析]**
**结论：** {一句话核心判断 — 如「引擎标签与中期趋势矛盾，以趋势为准」}
...
**[分析]**
**结论：** {一句话核心判断 — 如「广度与量能未同步，属情绪脉冲而非放量突破」}
...
**[分析]**
**结论：** {一句话核心判断 — 如「各路资金方向分化，属结构分歧而非共识」}
...
**[事实]**
- {macro_signal_label 两段摘要**整体原样引用**（v0.3.0 起为两行：首行「国内：PMI/CPI/LPR/M2 →政策方向 |」，次行「海外：VIX/SOX/美 10Y/布油等 →海外结论」）——**不得只取首行**，否则丢失全部海外锚点与结论；全部不可得时为单行「宏观数据不可得」}
...
**[分析]**
**结论：** {一句话核心判断 — 如「国内政策偏宽松、海外利率高位压制未解除，两段分开陈述」}
...
**[分析]**
**结论：** {一句话核心判断 — 如「四信号指向去杠杆末段的企稳确认，方向可信但精度受窗口限制」}
...
**[分析]**
**结论：** {一句话核心判断 — 如「显著负相关共享同一对手方且无同步池，指向存量腾挪」}
...
4. **P0 规则**：所有分位/比率/变化率必须由 Python 计算后引用，禁止 AI 心算；
   **计数（N 个/N 家/N 日/N 个行业）与区间/极值断言（峰值/最大/最少/首个/最长）同样禁止目视**——
   引擎输出的清单不是聚合，须对其跑 `len()` / `max` / `min` / 分位聚合后再引用；
...
```bash
cd "${INVEST_SKILLS_ROOT:-.}" && uv run python skills/lib/report_qc.py reports/market-pulse/{YYYY-MM-DD}.md --fail-on error
```

- 退出码 **0=PASS / 1=WARN 均可交付；2=FAIL 不得交付**，须修正后重跑至非 FAIL。
- sourcing warning（F2 派生词缺来源 / F4 §N 引用不存在）逐条复核后消除或说明。
- **其余 warning 同样逐条判定**：区分「报告缺陷」（改报告）与「规则误报」（在报告内标注豁免理由，勿静默忽略）。
- 本层先于人工复检——**机器能判的不要靠人看**。
...
- [ ] **第 0 层机器准出已跑**：`report_qc.py <报告> --fail-on error` 退出码非 2（PASS/WARN 可交付），warning 已逐条判定（改报告 or 标注豁免理由）
...
- [ ] 每个 `[分析]` 块首句为 `**结论：**` 加粗主旨句（字面量，冒号后紧接 `**`），且该段含 `**逻辑链：**`
- [ ] 无目视计数 / 子集区间断言：所有计数（N 个/N 家/N 日）与极值（峰值/最大/首个）已对全量序列跑 `len()`/`max`/`min` 聚合
- [ ] PE/PB 分位所在行**同线**带中位数或均值（lint `percentile-without-median` 按行判定）
- [ ] 推测性规律标注「待验证」

...
def test_aligned_nav_returns_small_dividend_fixed():
    """修复 #1（A2）：小分红（除权 <15%）不再漏修。

    旧逻辑硬阈值 `abs(cur_nav/prev_nav - 1) >= 0.15` 对 2% 分红不触发 →
    假跳变残留；新逻辑感知除权幅度先验（expected_drop），小分红同样采用新因子。
    """
    adj_map = {"20260701": 1.0, "20260702": 1.0, "20260703": 1.0204, "20260706": 1.0204}
    # 净值源提前除权：07-02 净值 = 1.0 × 0.98 × (1+0.5%) ≈ 0.9849（拆后口径，因子仍 1.0）
    df = pd.DataFrame([
        {"净值日期": "2026-07-01", "单位净值": 1.0000, "日增长率": 0.0},
        {"净值日期": "2026-07-02", "单位净值": 0.9849, "日增长率": -1.51},
        {"净值日期": "2026-07-03", "单位净值": 0.9860, "日增长率": 0.11},
        {"净值日期": "2026-07-06", "单位净值": 0.9890, "日增长率": 0.30},
    ])
    navs, returns, rows = _aligned_nav_returns(df, adj_map=adj_map)
    # 前复权除最新因子（1.0204）：07-01 → 0.9800；07-02 采用新因子 → 0.9849
    # （旧逻辑 navs[1] = 0.9849/1.0204 = 0.9652 假低点）
    assert navs == pytest.approx([0.9800, 0.9849, 0.9860, 0.9890], abs=1e-3)
    assert returns == pytest.approx([0.0050, 0.0011, 0.0030], abs=1e-3)


def test_aligned_nav_returns_small_dividend_with_market_drop():
    """修复 #1：小分红 + 同日市场下跌 4%（同向）仍采用新因子。"""
    adj_map = {"20260701": 1.0, "20260702": 1.0204, "20260703": 1.0204}
    df = pd.DataFrame([
        {"净值日期": "2026-07-01", "单位净值": 1.0000, "日增长率": 0.0},
        # 除权 2% + 市场跌 4%：净值 = 1.0 × 0.98 × 0.96 ≈ 0.9408
        {"净值日期": "2026-07-02", "单位净值": 0.9408, "日增长率": -5.92},
        {"净值日期": "2026-07-03", "单位净值": 0.9502, "日增长率": 1.00},
    ])
    navs, returns, _ = _aligned_nav_returns(df, adj_map=adj_map)

...
version: "0.2.0"
description: "交易日志 v2 — Claude 驱动四维评估（逻辑/盲点/仓位匹配/风险收益）+ 数据引擎 + ETF 对冲检查。研究工具，非决策工具。"
...
2. **数据查询**：调用 Python 引擎脚本查询 PE 分位、波动率、宏观、两融、涨跌比、涨跌停比
...
Claude: 交互式提问（一次 3 题；AskUserQuestion 若可用，否则对话提问）
  ...
  Q0c. 哪只标的？    → 常用ETF点选 / 其他(自定义输入代码)
       ↓
  ┌─ ETF 路径 ─────────────────────────────────────┐
  │  调 etf_data.py query_etf_data(代码)            │
...
| `etf_data.py` → `query_etf_data(symbol)` | 指数PE/折溢价/AUM/对冲 | ETF 评估 |
...
两融/涨跌比/涨跌停比/ETF 数据由 journal 侧直接调 akshare（不经过 invest-a-stock collector）。
...
- `references/etf-hedge-map.md` — 主要指数 ETF 期货/期权对冲工具覆盖表

...
argument-hint: "/limit-up | /limit-up --sector 半导体 | /limit-up 600176 深挖"
...
用户选择标的后，运行 invest-A 采集深挖：

```bash
uv run python skills/invest-A/scripts/invest.py collect SYMBOL --with-news-pack
```
...
### [技术前兆]
- 涨停前 K 线形态、均线排列、量价关系
- 参考 invest-A technical.py 分析

### [资金行为]
- 北向/主力/融资融券行为
- 参考 invest-A participant_scan.py 分析
...
| 用户输入 | Claude 行为 |
|----------|------------|
| `/limit-up` | 全扫描 + 简报 + 引导提问 |
| `/limit-up --sector 半导体` | CLI 加 `--sector 半导体` 重新扫描 |
| `连板/2板/3板` | CLI 加 `--min-board N` |
| `深挖 SYMBOL` | 运行 invest-A collect + 输出归因卡 |

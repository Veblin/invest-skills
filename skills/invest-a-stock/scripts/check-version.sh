...
# ============================================================
# check-version.sh — invest:a-stock 版本一致性检查
# canonical: pyproject.toml
# 调用 sync_version.py check 校验，并检测 SKILL.md 缓存路径
# ============================================================
...
if [[ -z "$SKILL_FILE" ]]; then
    SKILL_FILE="$REPO_ROOT/skills/invest-a-stock/SKILL.md"
fi
...
if [[ "$CACHE_STATUS" == "cache" ]]; then
    echo "⛔ invest:a-stock 从缓存路径加载 SKILL.md"

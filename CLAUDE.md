...
**所有涉及数值的计算（加减乘除、百分比、均值、偏离度、比值、计数等）必须由 Python 引擎完成，AI 不得在脑中或对话中进行任何数学计算。**
...
7. **"检索不到"就推断"数据不存在/可能是假的"** — 违反事实边界 §2.3。检索无结果只能标注「未检索到」/「公开不可独立验证」，不得推断为不存在/停更/数据缺失原因。
...
提取报告全部关键数字，按来源分组逐项对照：

| 来源分组 | 对照内容 |
|---------|---------|
| 引擎字段 | 对照原始 JSON 输出，值/单位/口径必须一致 |
| Python calc | 对照计算输出，标注 `[来源: Python calc: formula]` |
| 检索/新闻口径 | 必须带「检索摘要口径，出处待核实」标注；**不得归因到未读原文的媒体** |

- 极值断言（峰值/最大/最低/首个）→ 确认基于**全量序列**的 Python 聚合
- 任何不匹配 → 修正报告后再进入第 2 层

**已知错误实例（不得重犯，2026-08-07 588000 报告审计）**：
1. **子集断言**：只看份额流近 8 行就断言"7/30 +59.01 亿为峰值"，真实峰值是 7/20 +138.16 亿
2. **摘要归因**：把 WebSearch 摘要里的"半导体权重 65%/73.8%"归因到"21经济网/东方财富"，实际未读原文
3. **目视计数**：数"跌幅 ≥5% 交易日"得 7 个（实际 6 个）；数"净流出日"得 3 个（实际 4 个）

### 第 2 层：合规复检（规则）

- **P0**：每个数字有引擎来源或 `[来源: Python calc: formula]`，无 AI 心算/目视计数痕迹
- **LAW 6/6a**：无买卖/仓位建议；多情景带假设前提+概率权重；无无假设单一目标价
- **措辞规范 §3**：禁止词替换表 + 已知违规模式 7 条逐一核对

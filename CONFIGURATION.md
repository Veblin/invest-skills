...
```bash
npx skills add Veblin/invest-A-skill --skill invest-A -g -a cursor -y
```

### OpenClaw

```bash
npx skills add Veblin/invest-A-skill --skill invest-A -g -a openclaw -y
# 或（已 clone 本仓库时）
openclaw skills install git:Veblin/invest-A-skill
```

### Hermes / Codex / 其他 Agent Skills 运行时

```bash
npx skills add Veblin/invest-A-skill --skill invest-A -g -y

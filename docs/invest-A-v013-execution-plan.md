...
> 日期: 2026-06-14（v3：默认 Markdown 输出，HTML 延后）
...
| 阶段 | 工期 | 核心交付 | 可验证标准 |
|------|------|---------|-----------|
| **P0** | 1-2 周 | 动态驱动 + 左侧/右侧 + 交叉验证 + Markdown 9 模块 | 报告能回答"现在为什么涨跌、左侧还是右侧、什么条件会变" |
| **P1** | 2-4 周 | 分层激活基本面 + 风险扫描 + 隐含增长率 + PE Band 数据层 | 12 核心题强制执行，17 风险信号自动扫描 |
| **P2** | 4-6 周 | Bull/Bear 辩论 + 跨时点 diff + 批量运行 + 情绪增强 | 多股票对比，趋势变化感知 |
| **P3** | 延后 | PE Band 文本表增强 + Mermaid 流程图（Markdown 内） | 不阻塞 v0.1.3 发布 |
| **稳定后** | TBD | HTML 9 模块 + ECharts + 章节导航 | 报告结构冻结后再投入 |
